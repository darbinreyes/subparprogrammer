...
If translation is enabled the controller will translate "scan code set 2" into "scan code set 1".
// [x] Tested. Expects Data: Yes. 0x00=Get scan code set number. Response:  ACK, then the scan code set number. // See spec. for other responses.
...
DISABLE_SCANNING = 0xF5, // [x] Tested. Expects Data: No. Response: default.
...
Some PS/2 keyboard commands require data to be sent after the command is sent.

*/
typedef
enum _ps2_kbd_cmd_data_t {
    GET_SCAN_CODE_SET = 0x00,
    SET_SCAN_CODE_SET_1 = 0x01,
    SET_SCAN_CODE_SET_2 = 0x02,
    SET_SCAN_CODE_SET_3 = 0x03
}  ps2_kbd_cmd_data_t;
