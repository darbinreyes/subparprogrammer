...
/*
  ...
  login function. See secret page code along pt. 4. Tell passport to "for the
  local strategy use that method for the User.authenticate()".
*/
passport.use(
...
/*
  ...
  hiding/showing of the login/sign-up/logout links in the navbar based on
  whether or not the user is logger in. It also allows us to display the name of
  the currently logged in user in the navbar. This is really express setup but
  I'm pretty sure this line must occur after passport setup. Without this we
  would have to pass req.user to exp_response.render() almost every time it is
  called in a route.
*/
app.use(function(req, resp, next){
  resp.locals.currentUser = req.user; // See "currentUser" in header.ejs.
  resp.locals.errorMessage = req.flash("error"); // See "errorMessage" in header.ejs.
  resp.locals.successMessage = req.flash("success"); // See "successMessage" in header.ejs.
  next();
});
...
/*
  ...
  routes with the string provided in the first argument. This way we avoid
  errors caused by a mistyped route path string. Note that if a route parameter
  like ":id" occurs in this prefix you must use
  "express.Router({mergeParams: true});". See routes/comments.js.
*/
