  ...
  User.register(
    ...
    function(err, registeredUser) {
      ...
      /*
        ...
        FYI: "local" strategy can be swapped with e.g. "twitter". See Secret
        page code along pt.3. "This actually logs in the user".
      */
...
/*
  The call to passport.authenticate() here is called "middleware". Middleware =
  "code that runs  before our final route callback". Middleware can be "stacked
  up".
*/
...
router.get("/logout", function(exp_request, exp_response){
  console.log(exp_request.method + " @ " + exp_request.originalUrl);
  /*
    This method is from passport, it destroys the user's session.
    Logs out the user.
    How does this function know which user to log out? ANS: The user info is
    inside exp_request.
  */
  exp_request.logout();
